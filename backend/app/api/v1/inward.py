...
async def create_inward_log_entry(
    ...
    await create_audit_log(
        ...
        AuditLogCreate(
            ...
            field_changed=None,
            old_value=None,
            new_value=json.dumps(sa_obj_to_dict(db_log))
        )
    )
    return db_log
...
async def update_inward_log_entry(
    ...
    await create_audit_log(
        ...
        AuditLogCreate(
            ...
            entity_id=log_id,
            field_changed=None,
            old_value=json.dumps(sa_obj_to_dict(old_log) if old_log else None),
            new_value=json.dumps(sa_obj_to_dict(db_log))
        )
    )
    if db_log is None:
...
async def delete_inward_log_entry(
    ...
    await create_audit_log(
        ...
        AuditLogCreate(
            ...
            entity_id=log_id,
            field_changed=None,
            old_value=json.dumps(sa_obj_to_dict(db_log)) if db_log else None,
            new_value=None
        )
    )
    if db_log is None:
